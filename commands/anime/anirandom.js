...
module.exports = {
  ...
  run: async ( client, message ) => {
    ...
    // Indicator that Kei is trying to fetch these data
    message.channel.startTyping();

    const db = animeDB.filter(a => message.channel.nsfw === a.isAdult);
    const { ids: { al: id }} = db[Math.floor(Math.random() * db.length)];

    const { errors , data } = await client.anischedule.fetch(`query ($id: Int) { Media(id: $id){ siteUrl id idMal synonyms isAdult format startDate { year month day } episodes duration genres studios(isKein:true){ nodes{ name siteUrl } } coverImage{ large color } description title { romaji english native userPreferred } } }`, { id });

    const embed = new MessageEmbed().setColor('RED')
    .setFooter(`Random Recommendations | \©️${new Date().getFullYear()} Kei`);

    // If errored due to ratelimit error
    if (errors && errors.some(x => x.status === 429)){
      return message.channel.send(
        embed.setAuthor('Oh no! Kei has been rate-limited', 'https://cdn.discordapp.com/emojis/767062250279927818.png?v=1')
        .setDescription([
          `**${message.member.displayName}**, please try again in a minute.\n\n`,
          `If this error occurs frequently, please contact **SakurajiKei#6742**.`
        ].join(''))
      );
    };
    ...
    if (errors && errors.some(x => x.status === 400)){
      return message.channel.send(
        ...
        .setDescription([
          ...
          `Please contact **SakurajiKei#6742** for a quick fix.\n`,
          `You can make an issue on the [repository](${client.config.github}) or [join](https://support.Kei-san.ml/) Kei's dev server instead.`
        ].join(''))
      );
    };
    ...
    if (errors){
      return message.channel.send(
        ...
        .setDescription([
          ...
          `If this doesn't resolve in few hours, you may contact **SakurajiKei#6742**`,
          `You can also make an issue on the [repository](${client.config.github}) or [join](https://support.Kei-san.ml/) Kei's dev server instead.`
        ].join(''))
      );
    };
