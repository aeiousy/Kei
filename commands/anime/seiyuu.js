...
module.exports = {
  ...
  run: async ( client, message, args) => {
    ...
    .setFooter(`Seiyuu Query with AL | \©️${new Date().getFullYear()} Kei`)
    ...
    let Keinpage = await message.channel.send(embed);
    ...
    if (res.errors && res.errors.some(e => e.message !== 'Not Found.')){
      ...
      return await Keinpage.edit(embed).catch(()=> null) || message.channel.send(embed);
    };
    ...
    .setFooter([
      ...
      `Seiyuu Query with AL | \©️${new Date().getFullYear()} Kei`
    ].join('\u2000•\u2000'));

    return await Keinpage.edit(embed).catch(()=>null) || message.channel.send(embed).then(()=>null);
