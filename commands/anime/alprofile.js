  ...
  run: async function ( client, message, args ) {
    ...
    if (response.errors){
      ...
      if (response.errors[0].status === 404){
        ...
        err =`\`❌ CLIENT_ERR\`: Kei attempted to send an invalidated request to AniList. Please contact my developer to fix this bug.`;
      } else {
    ...
    return message.channel.send(
      ...
      .setFooter(`ALProfile | \©️${new Date().getFullYear()} Kei`)
