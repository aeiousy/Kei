    ...
    if (!isActive){
      return message.channel.send(
        embed.setDescription([
          ...
          `[Learn More](https://Kei-san.ml/docs/features/XP_System) about Kei's XP System.`
        ].join('\n'))
    ...
    return profile.find({ 'data.xp.id': message.guild.id }, async (err, docs) => {
      if (err) {
        return message.channel.send(
          ...
          .setDescription('Kei\'s Database Provider responded with an error: ' + err.name)
        );
      };
      ...
      if (!docs.length){
        return message.channel.send(
          embed.setDescription([
            ...
            '[Learn More](https://Kei-san.ml/) about Kei\'s Economy System.'
          ].join('\n'))
      ...
      return message.channel.send(
        new MessageEmbed()
        .setColor('GREY')
        .setFooter(`Credits Leaderboard | \©️${new Date().getFullYear()} Kei`)
