...
module.exports = {
  ...
  run: (client, message) => list.findById(message.guild.id, async (err, doc) => {
    ...
    .setColor('RED')
    .setFooter(`Anischedule Watchlist | \©️${new Date().getFullYear()} Kei`)
    ...
    if (!anischedch){
      ...
      const pages = new Page(descriptions.map((d,i) => {
        ...
        .setFooter([
          ...
          `\©️${new Date().getFullYear()} Kei`
        ].join('\u2000\u2000•\u2000\u2000'))
        .addField('Tips', [
          `- Use [\`${client.prefix}watch\`](https://Kei-san.ml/docs/Features/Anischedule#adding-more-to-the-list) to add subscription`,
          `- Use [\`${client.prefix}unwatch\`](https://Kei-san.ml/docs/Features/Anischedule#removing-anime-from-the-list) to remove subscription`,
