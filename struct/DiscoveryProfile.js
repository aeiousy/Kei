...
module.exports = class DiscoveryProfile {
   ...
   */
  _fetch(ids){
    return anilist.fetch('query ($ids: [Int]) { Page{ media(id_in: $ids){ siteUrl id idMal type isAdult format startDate { year month day } episodes duration chapters volumes genres studios(isKein:true){ nodes{ name siteUrl } } coverImage{ large color } description title { romaji english native userPreferred } } } }', { ids });
  };
