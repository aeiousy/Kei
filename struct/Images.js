...
const Kei = require(`${process.cwd()}/assets/json/Kei.json`);
...
module.exports = class ImageGenerator{
  constructor(){
    ...
    /**
		 * Array of Kei images imported from assets/json/Kei.json
		 * @type {Kei[]}
     * @private
     */
    this._Kei = Kei;
...
/**
  ...
  */
  Kei(r = {}){
    ...
    const index = Math.floor(Math.random() * this._Kei[type].length);

    return this._Kei[type][index];
  };
  ...
  */
  get size(){
    ...
    for (const s of Object.keys(this._Kei)){
      profiles.Kei[s] = this._Kei[s].length
    };
